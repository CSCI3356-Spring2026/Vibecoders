...
from .models import RoommateGroup, RoommateGroupInvite, RoommateGroupInviteApproval, RoommateGroupMembership
...
def remove_group_member(*, acting_user, membership):
    ...
    with transaction.atomic():
        ...
        if group.lead_id != acting_user.id:
            raise ValidationError("Only the group lead can remove members.")
...
def _create_invite_approvals(invite, inviter):
    approvals = []
    now = timezone.now()
    for membership in roommate_group_memberships(invite.group):
